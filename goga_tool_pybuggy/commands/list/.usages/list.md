# pybuggy.commands.list — команда endpoint list

## Предметная область

Шаблоны потребления cell `pybuggy/commands/list`: вывод эндпоинтов, сгруппированных по spec. Аудитория — регистрация в CLI (`list_cmd`) и тесты (`run_list` напрямую).
...
Конфиг грузится из фиксированного пути через `load_config()`. Формат задаёт `pybuggy.output.render_list`.
