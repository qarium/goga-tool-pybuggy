...
`.goga/tools/pybuggy/config.yml` **всегда перегенерируется** на каждом вызове `pybuggy init` — проверки существования
файла и подтверждения перезаписи (y/N) нет. Существующий файл перезаписывается свежими ответами опроса; шаг завершается
exit 0, остальной `init` продолжается.
