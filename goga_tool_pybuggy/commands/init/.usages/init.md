...
Перезапись: `.goga/tools/pybuggy/config.yml` **всегда перегенерируется** на каждом вызове `pybuggy init` — проверки
существования файла и подтверждения перезаписи (y/N) нет. Предыдущее содержимое заменяется свежими ответами опроса;
остальной `init` продолжается (exit 0).
