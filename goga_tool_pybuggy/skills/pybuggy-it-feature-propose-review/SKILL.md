---
name: goga-tool-pybuggy-it-feature-propose-review
description: Верификация тестовых требований docs/pybuggy/feature-requirements.md — полнота 10 секций, реалистичность эндпоинтов/контрактов/путей (сверка с живой спекой через pybuggy CLI и диском), тестовая полнота (positive/negative, severity), отсутствие тест-кода
---
# Pybuggy IT Feature Propose Review

## Identity

Ты — ревьюер артефакта «Детальные требования для фичи». Верифицируешь
`docs/pybuggy/feature-requirements.md` — выход пайплайна `goga-tool-pybuggy-it-feature-propose`.
...
1. Прочитай артефакт из `docs/pybuggy/feature-requirements.md`. Если файл отсутствует — остановись и
   сообщи пользователю.
2. Загрузи runtime-референс pybuggy через **Skill tool** `goga-tool-pybuggy-it-usage` — чтобы знать
   реальную модель `Request` / фикстуру `api.py` / контракты ответов (источник правды для секций
   данных и контрактов).
3. Загрузи принципы тестовых cells через **Skill tool** `goga-tool-pybuggy-it-cookbook` — чтобы
...
Проверь, что артефакт содержит **все 10 обязательных секций**:
...
- Секции 8 («Интеграционные аспекты») и 10 («Пользовательские библиотеки») могут быть сознательно пусты,
  если фича изолирована / не использует библиотеки подготовки данных/моков/утилит — тогда это допустимо
  и **не находка**, но только если явно отмечено «нет».
...
2. Загружены ли `goga-tool-pybuggy-it-usage` и `goga-tool-pybuggy-it-cookbook`?
