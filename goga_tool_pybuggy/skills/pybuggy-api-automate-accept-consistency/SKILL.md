...
2. Шаги `Steps:` аннотации Routine отражены в теле теста (вызовы, проверки); пропущенный шаг кейса —
   находка (Severity по влиянию: потерянная проверка контракта — High).
3. Проверки кейса (статус, поля, структура, инварианты) присутствуют в ассертах — сверить с разделом
