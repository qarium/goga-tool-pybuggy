...
основание | решение пользователя | план-направление.
...
3. `service-bug` — задача на каждый упавший тест: баг-запись адресуется к одному тесту.
...
- Проверка: rerun затронутых тестов — симптомы класса environment ушли (connection/env/сеть). Тест, оставшийся
...
Проверка: `goga tool pybuggy endpoint diff <endpoint-id> [...]` — пустой; `goga lint` клетки;
...
Проверка: `goga lint` клетки; `pytest tests/<spec>/<id>/ -q` — зелёный.
...
- Проверка: `pytest tests/<spec>/<id>/ -q` — зелёный.

### `service-bug` — записать баг сервиса

- Действия: допиши в `docs/bugs/<feature>.md` запись `BUG-<feature>-<N>` (сквозная нумерация) по полному шаблону
  исполнителя `goga-tool-pybuggy-api-fix-execute-bug`; материал — секция «Досье и доказательства» артефакта анализа и
  traceback из `docs/fix/<feature>-log.txt`.
- Проверка: запись создана; тест остаётся красным — честный исход.
