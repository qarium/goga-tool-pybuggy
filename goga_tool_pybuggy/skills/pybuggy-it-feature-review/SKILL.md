---
name: goga-tool-pybuggy-it-feature-review
...
---
# Pybuggy IT Feature Review (dispatcher)
...
`goga-tool-pybuggy-it-feature-*-review`.
...
2. **Есть** — вызови через **Skill tool** `goga-tool-pybuggy-it-feature-propose-review`.
...
2. **Есть** — вызови через **Skill tool** `goga-tool-pybuggy-it-feature-testcases-review`.

#### cells
Проверь, что `docs/pybuggy/feature-cells.md` существует.
1. **Нет** — остановись и сообщи пользователю (сначала нужен пайплайн `cells`).
2. **Есть** — вызови через **Skill tool** `goga-tool-pybuggy-it-feature-cells-review`.

#### design
Проверь, что `docs/design/<target>.md` существует.
1. **Нет** — остановись и сообщи пользователю.
2. **Есть** — вызови через **Skill tool** `goga-tool-pybuggy-it-feature-design-review`, передав `<target>`.

#### plan
Проверь, что `docs/plans/<target>.md` существует.
1. **Нет** — остановись и сообщи пользователю.
2. **Есть** — вызови через **Skill tool** `goga-tool-pybuggy-it-feature-plan-review`, передав `<target>`.

#### cell
Это материализованная тестовая cell (`tests/<spec>/<id>/`). Её архитектурная верификация — зона пайплайна
`cells`. Направь пользователя к `goga-tool-pybuggy-it-feature-cells-plan-verification` (внутрипайплайнный
