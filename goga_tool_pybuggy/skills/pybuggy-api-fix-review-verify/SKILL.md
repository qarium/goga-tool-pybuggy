...
2. `record-bug` — записи созданы в `docs/bugs/<feature>.md`, тесты честно красные.
3. Финальный прогон (`docs/fix/<feature>-log.txt`) сходится с ожиданием: зелёные все, кроме `service-bug` тестов с
   баг-записями.
...
4. `docs/bugs/<feature>.md` — полный шаблон записей, сквозная нумерация.
