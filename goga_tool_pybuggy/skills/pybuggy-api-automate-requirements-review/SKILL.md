...
10. **Доступные usages проекта** — таблица `ключ | путь | роль | назначение` — реестр скана `.goga/usages/`
...
- Секции 8 («Интеграционные аспекты») и 11 («Уже покрыто тестами») могут быть сознательно пусты, если фича
  изолирована / покрытие отсутствует — тогда это допустимо и **не находка**, но только если явно отмечено
  «нет» / «покрытие отсутствует».
- Секция 10 отражает скан диска: если `.goga/usages/` пуст — явная отметка «usages отсутствуют»
...
8. **Ограничения** — секция «Ограничения и границы» описывает, что фича **не делает**. Пустые общие
   фразы — **Medium**.
9. **Реестр usages (§10)** — сверить с диском: каждый ключ таблицы соответствует существующему файлу
   ...
   в §10, — **Medium** (реестр неполон); ключ без файла — **High** (висячая ссылка для `testcases`);
...
5. Проверена ли структурная полнота (все обязательные секции, без плейсхолдеров; §8/§11 опциональны,
   §10 — реестр с явной отметкой при пустых usages)?
...
7. Проверена ли консистентность (сценарий↔эндпоинт, severity, данные↔Request, контракты↔schemas,
   критерии приёмки)? Сверен ли реестр usages §10 с диском (ключ ↔ файл, роли)?
