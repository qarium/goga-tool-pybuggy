...
| `service-bug` | diff пуст; Routine и тест соответствуют контракту; сервис отвечает не по спеке | баг-запись `docs/bugs/<feature>.md`                  |
...
| `ambiguous`   | доказательств недостаточно                                                     | совместный разбор, открытый вопрос                   |
...
2. Спроси пользователя: подтвердить гипотезу / другой класс / неоднозначно.
3. Зафиксируй класс, основание и решение.
...
## Неоднозначные
[Падения класса ambiguous с открытыми вопросами. Пусто, если нет]
```
