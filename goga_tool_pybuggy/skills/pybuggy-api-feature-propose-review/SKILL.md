...
собрать из них конкретные кейсы, а `cells` — Routine на кейс. Ты **анализируешь** артефакт,
...
10. **Пользовательские библиотеки** — таблица `ключ | библиотека | назначение (данные/моки/утилиты) |
    где используется (endpoint-id / сценарий)`.
...
8. **Ограничения** — секция «Ограничения и границы» описывает, что фича **не делает**. Пустые общие
   фразы — **Medium**.
9. **Пользовательские библиотеки (§10)** — каждая упомянутая библиотека имеет назначение и хотя бы одну
   точку использования (`endpoint-id`/сценарий), согласованную с секциями 2 и 4. Ключ — короткое имя (файл
   `.goga/usages/cooks/<ключ>.md`). Библиотека без точки использования или с несогласованной ссылкой —
   **Medium**.
