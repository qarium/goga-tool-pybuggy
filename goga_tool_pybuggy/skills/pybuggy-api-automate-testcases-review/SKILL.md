...
6. **Инструменты (usage-ключи)** — каждый usage-ключ, упомянутый в Предусловиях кейсов, существует:
   либо в §10 `feature-requirements.md` (реестр доступных usages), либо как созданный файл
   `.goga/usages/cooks/<ключ>.md` (шаг `tools` пайплайна). Ключ без файла на диске и без записи в §10 —
...
5. Проверена ли трассируемость к требованиям (версия, эндпоинты, сценарии, критерии приёмки) и
   существование usage-ключей Предусловий (§10 / `.goga/usages/cooks/`)?
