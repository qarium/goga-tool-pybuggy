---
name: goga-tool-pybuggy-it-feature-cells-context
...
3. `goga-tool-pybuggy-it-usage` — референс потребления runtime pybuggy (`api`, `asserts`).

### Step 2. Зафиксировать базовый блок Header

Из `goga-codemanifest-base` взять единый для всех тестовых cells блок `Usages` (`conventions`, `pybuggy-api`,
`pybuggy-asserts`) + `Annotations`. Без `Imports`.
...
Из `goga-tool-pybuggy-it-usage` — способ вызова эндпоинта (сгенерированная фикстура `api/<spec>/<id>/api.py`,
