---
name: goga-tool-pybuggy-it-feature-plan
...
---
# Pybuggy IT Feature Plan (dispatch)
...
- **Runtime:** pybuggy `Api`/`Endpoint`/`ResponseWrapper` + assert-слой. Грузи `goga-tool-pybuggy-it-usage`,
...
2. Загрузи контекст через **Skill tool**: `goga-tool-pybuggy-it-usage`, `goga-tool-pybuggy-it-cookbook`, `goga-cell`,
   `goga-cell-python`.
...
2. **Tasks** содержат чекбокс запуска тестов, **исполнимый** (например
   `[ ] Run tests: pytest tests/<spec>/ -q — all must pass`). Никогда не помечай запуск тестов как
...
4. **Completion Criteria** включает «all pytest validation commands pass».
