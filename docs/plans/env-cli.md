...
- [x] Contract tests and logic tests cover facade, API, and behavior within each coding task (T1–T14); integration tests (Task 4: T12, T15) cover the cross-entity / cross-cell runtime coupling.
- [x] Integration tests exist for the cross-cell scenario (ROOT ↔ pull via `PYBUGGY_REF`).
...
- [x] All validation commands pass (`pytest tests/ -x`, `ruff check`, `ruff format`, facade import).
- [x] Every Usages entry is referenced in at least one task: `conventions` (Tasks 1–4), `click` (Tasks 2–4), `python-dotenv` (Task 2), `gitpython` (Task 4, mocked).
